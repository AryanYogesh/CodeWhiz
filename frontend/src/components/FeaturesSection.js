...
const FeaturesSection = () => {
  ...
  return (
    <section className="bg-gray-50 py-16 w-full relative">
      <div className="max-w-7xl mx-auto px-6">
        <h2 className="text-3xl font-bold text-center text-gray-800 mb-8">Why Choose CodeWhiz?</h2>

        {/* Scrollable Container */}
        <div className="relative">
          {/* Left Arrow */}
          <button 
            className="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gray-200 p-3 rounded-full shadow-md hover:bg-gray-300 transition"
            onClick={() => scroll("left")}
          >
          ...
          >
            {features.map((feature, index) => (
              <motion.div
                key={index}
                className="bg-white min-w-[250px] md:min-w-[300px] rounded-xl shadow-lg p-6 flex flex-col items-center text-center hover:shadow-2xl transition"
          ...
          {/* Right Arrow */}
          <button 
            className="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gray-200 p-3 rounded-full shadow-md hover:bg-gray-300 transition"
